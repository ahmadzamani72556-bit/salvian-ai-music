...
export default function GenerationMonitorV3() {
  ...
  useEffect(() => {
    ...
    const patchedFetch: typeof window.fetch = async (...args) => {
      const request = args[0]; const init = args[1]; const url = typeof request === "string" ? request : request instanceof Request ? request.url : ""; const method = (init?.method || (request instanceof Request ? request.method : "GET")).toUpperCase();
      let action = ""; if (typeof init?.body === "string") { try { action = String((JSON.parse(init.body) as Record<string, unknown>)?.action || ""); } catch {} }
      const generation = url.includes("/api/music") && method === "POST" && action !== "status" && action !== "balance";
      if (generation) { begin(); if (url.includes("/api/music/instrumental")) setCookie("salvian_generation_endpoint", "/api/music/instrumental"); else setCookie("salvian_generation_endpoint", "/api/music"); }
      const response = await originalFetch(...args);
      if (generation) { const clone = response.clone(); void clone.json().then((data: Record<string, unknown>) => { const id = data?.taskId ? String(data.taskId) : ""; const created = Number(data?.createdAt || 0) * 1000; if (id) { attach(id, created); void check(); } else if (data?.success === false) { setIsFailed(true); setStatus("failed"); } }).catch(() => {}); }
      return response;
    };

    document.addEventListener("click", pointer, true); window.fetch = patchedFetch; const existing = cookie("salvian_generation_task"); if (existing) attach(existing); void check(); poll = window.setInterval(() => void check(), 5000); clock = window.setInterval(() => { if (!stopped && current?.startedAt) { const seconds = Math.max(0, (Date.now() - current.startedAt) / 1000); setElapsed(seconds); if (seconds >= CONTROL_LIMIT_SECONDS && current.id !== "pending") { setIsFailed(true); setStatus("timeout"); clearCookie("salvian_generation_task"); } } }, 1000);
