...
export default function GenerationMonitorV3() {
  ...
  return <div className="fixed bottom-20 left-3 right-3 z-[70] mx-auto max-w-xl rounded-2xl border border-violet-400/30 bg-[#111116]/98 p-4 shadow-2xl backdrop-blur-xl"><div className="flex items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-violet-500/15 text-violet-200">{isFailed ? <AlertTriangle size={21}/> : done ? <CheckCircle2 size={21}/> : <LoaderCircle size={21} className="animate-spin"/>}</div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2 text-sm font-bold"><div className="flex items-center gap-2"><Music2 size={15}/>{isFailed ? "Pembuatan musik bermasalah" : done ? "Musik selesai" : stuck ? "Task Mureka masih tertahan" : "Pembuatan musik"}</div><div className="flex items-center gap-1 text-xs text-violet-200"><Clock3 size={13}/> {duration(elapsed)}</div></div><p className="mt-1 text-xs leading-5 text-zinc-400">{isFailed ? "Mesin musik melaporkan proses gagal. Kredit akan diproses kembali secara otomatis." : done ? "Musik selesai dan project diperbarui di Library." : stuck ? "Task sudah lebih dari 20 menit. SALVIAN AI MUSIC tetap memantau Task ID ini; jangan membuat ulang dulu agar tidak terjadi pemotongan kredit ganda." : slow ? "Proses lebih lama dari biasanya. Tetap dipantau sampai mesin memberi hasil." : "Proses dipantau otomatis setiap 5 detik."}</p>{!done && !isFailed && <><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-violet-400 transition-[width] duration-1000" style={{ width: `${Math.min(100, Math.max(2, elapsed / NOTICE_AFTER_SECONDS * 100))}%` }}/></div><div className="mt-1 flex justify-between text-[9px] text-zinc-600"><span>{stuck ? "Task tetap dipantau" : slow ? "Masih dipantau" : "Kontrol waktu"}</span><span>{duration(Math.max(NOTICE_AFTER_SECONDS, elapsed))}</span></div></>}<div className="mt-2 flex items-center justify-between text-[10px] uppercase tracking-wider text-zinc-500"><span>{stage(status)}</span><span>{taskId === "pending" ? "Menunggu Task ID" : `Task ${taskId}`}</span></div>{lastCheck > 0 && !done && !isFailed && <div className="mt-1 text-[9px] text-zinc-600">Pemeriksaan terakhir: {new Date(lastCheck).toLocaleTimeString("id-ID")}{retrying ? " · koneksi sedang dicoba ulang" : ""}</div>}</div></div></div>;
}
