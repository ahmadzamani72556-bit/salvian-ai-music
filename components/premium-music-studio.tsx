...
export default function PremiumMusicStudio() {
  ...
  const uploadFile = async (file: File, purpose: "reference" | "voice") => {
    ...
    try {
      ...
      if (purpose === "reference") { setReferenceId(String(data.id)); setReferenceName(file.name); }
      else { setVocalId(String(data.vocal_id)); setVocalName(file.name); }
      setNotice("Berhasil. File siap dipakai pada pembuatan musik.");
    } catch (e) { setNotice(e instanceof Error ? e.message : "Upload gagal."); }
  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/music", {
        ...
        body: JSON.stringify({ title, lyrics: lyrics.trim(), style, model: modelMap[model], n: 1, instrumental, gender, reference_id: referenceId || undefined, vocal_id: vocalId || undefined }),
      });
      const data = await res.json().catch(() => ({}));
      ...
      await loadLibrary();
      setActive("library");
    } catch (e) { setNotice(e instanceof Error ? e.message : "Gagal membuat musik."); }
    finally { setBusy(false); }
  };
  ...
  const ask = async (event?: FormEvent) => {
    ...
    setChatInput(""); setMessages(prev => [...prev, { role: "user", content: message }]); setChatBusy(true);
    try {
    ...
    } catch (e) { setMessages(prev => [...prev, { role: "assistant", content: e instanceof Error ? e.message : "Terjadi kesalahan." }]); }
  ...
  const generateLyrics = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "Lirik Assistant gagal.");
      setLyrics(String(data.answer || "")); setNotice("Lirik berhasil dibuat oleh Lirik Assistant.");
    } catch (e) { setNotice(e instanceof Error ? e.message : "Lirik Assistant gagal."); }
      ...
      <div className="relative flex min-h-screen">
        <aside className="hidden w-[245px] shrink-0 border-r border-white/10 bg-[#050610]/95 lg:flex lg:flex-col">
          <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
            <div className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-violet-500 via-fuchsia-500 to-blue-500 shadow-[0_0_35px_rgba(168,85,247,.5)]"><Headphones size={27}/></div>
            <div><div className="text-xl font-black">SALVIAN <span className="text-violet-300">AI</span></div><div className="text-[9px] tracking-[.32em] text-zinc-500">MUSIC STUDIO</div></div>
          </div>
          <div className="flex-1 overflow-y-auto p-3">
            {nav.map(([id, label, Icon]) => <button key={id} onClick={() => id === "assistant" ? setChatOpen(true) : setActive(id)} className={`mb-1 flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition ${active === id ? "bg-gradient-to-r from-violet-600/80 to-fuchsia-500/50 text-white shadow-lg" : "text-zinc-400 hover:bg-white/5 hover:text-white"}`}><Icon size={19}/><span>{label}</span>{id === "assistant" && <span className="ml-auto rounded-full border border-violet-400/40 px-1.5 py-0.5 text-[8px] text-violet-200">PRO</span>}</button>)}
          </div>
        ...
        <main className="min-w-0 flex-1 overflow-y-auto pb-24">
          <header className="sticky top-0 z-30 flex items-center justify-between border-b border-white/10 bg-[#05050c]/85 px-4 py-4 backdrop-blur-xl sm:px-7">
            <div className="lg:hidden"><div className="font-black">SALVIAN <span className="text-violet-300">AI</span></div><div className="text-[8px] tracking-[.3em] text-zinc-500">MUSIC STUDIO</div></div>
            <div className="hidden lg:block"><div className="text-[10px] font-bold uppercase tracking-[.3em] text-violet-300">AI Music Creation</div><div className="text-xl font-black">Studio AI Musik Kelas Premium</div></div>
            <div className="flex items-center gap-2">
              <div className="hidden items-center gap-2 rounded-2xl border border-violet-400/25 bg-violet-500/5 px-3 py-2 sm:flex"><span className="text-amber-300">◆</span><b>{credits ?? 0}</b><span className="text-xs text-zinc-500">kredit</span><button onClick={() => setActive("monetize")} className="grid h-7 w-7 place-items-center rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-500"><Plus size={15}/></button></div>
              <div className="rounded-2xl border border-fuchsia-400/30 bg-fuchsia-500/10 px-3 py-2"><div className="flex items-center gap-1 text-xs font-black"><Crown size={14} className="text-amber-300"/> {plan === "FREE" ? "Premium" : plan}</div><div className="text-[9px] text-zinc-500">Studio Elite</div></div>
              <button onClick={() => setActive("account")} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-zinc-900"><UserRound size={18}/></button>
            </div>
          </header>
          ...
          <div className="mx-auto max-w-[1180px] px-4 py-5 sm:px-7 lg:py-7">
            ...
            {active === "studio" && <>
              <section className="relative overflow-hidden rounded-[30px] border border-violet-400/20 bg-gradient-to-br from-[#17102f] via-[#0a0b18] to-[#080811] p-6 shadow-[0_25px_90px_rgba(67,30,130,.25)] sm:p-8">
              ...
              <section className="mt-5 rounded-[26px] border border-violet-400/15 bg-[#090a13]/95 p-4 sm:p-6">
                ...
                <textarea value={prompt} onChange={e => setPrompt(e.target.value.slice(0, 2000))} className="min-h-[130px] w-full rounded-2xl border border-white/10 bg-black/25 p-4 text-sm leading-7 outline-none placeholder:text-zinc-600" placeholder="Jelaskan genre, tema, suasana, instrumen, tempo, bahasa, dan karakter vokal…"/>
                <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{presets.map(x => <button key={x} onClick={() => setPrompt(`${x}, kualitas studio premium, aransemen modern, vokal emosional, bass dan drum seimbang.`)} className="shrink-0 rounded-full border border-violet-400/20 bg-violet-500/5 px-3 py-2 text-[11px] font-semibold text-violet-200">{x}</button>)}</div>
              </section>
        ...
        <aside className={`fixed right-3 top-20 z-50 w-[min(360px,calc(100vw-24px))] transition lg:sticky lg:top-[80px] lg:z-20 lg:mr-5 lg:mt-[80px] lg:h-[calc(100vh-110px)] lg:w-[330px] ${chatOpen ? "" : "pointer-events-none opacity-0 lg:pointer-events-auto lg:opacity-100"}`}>
          <div className="flex h-[calc(100vh-110px)] max-h-[760px] flex-col overflow-hidden rounded-[26px] border border-violet-400/20 bg-[#080912]/95 shadow-[0_20px_80px_rgba(0,0,0,.55)] backdrop-blur-xl">
            <div className="flex items-center justify-between border-b border-white/10 p-4"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-violet-500/15 text-violet-200"><Bot size={23}/></div><div><div className="font-black">Asisten AI</div><div className="text-[10px] text-violet-300">Partner Kreatif Anda</div></div></div><button onClick={() => setChatOpen(false)} className="rounded-xl p-2 text-zinc-500 hover:bg-white/5 lg:hidden"><X size={17}/></button></div>
            <div className="flex-1 space-y-3 overflow-y-auto p-4">{messages.map((m,i) => <div key={i} className={m.role === "user" ? "flex justify-end" : "flex justify-start"}><div className={m.role === "user" ? "max-w-[90%] rounded-2xl rounded-br-md bg-violet-600/30 px-3 py-2 text-xs leading-5" : "max-w-[95%] rounded-2xl rounded-bl-md border border-white/10 bg-black/20 px-3 py-2 text-xs leading-5 text-zinc-300"}>{m.content}</div></div>)}{chatBusy && <div className="text-xs text-zinc-500">Asisten sedang mengetik…</div>}</div>
