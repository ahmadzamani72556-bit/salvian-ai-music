...
import { LoaderCircle, Music2, CheckCircle2, AlertTriangle } from "lucide-react";
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 text-sm font-bold">
            <Music2 size={15} />
            {failed ? "Pembuatan lagu gagal" : done ? "Lagu selesai" : "Pembuatan lagu masih berjalan"}
          </div>
          <p className="mt-1 text-xs leading-5 text-zinc-400">
            {failed ? "Mesin musik melaporkan proses gagal. Silakan cek Library; permintaan berikutnya tidak akan dianggap selesai sebelum hasil tersedia." : done ? "Hasil sudah tersedia dan project tetap tersimpan di Library." : "Mesin musik sedang memproses lagu. Anda boleh berpindah halaman atau refresh; proses tetap dipantau."}
          </p>
          {!done && !failed && <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full w-1/3 animate-[pulse_1.5s_ease-in-out_infinite] rounded-full bg-violet-400" /></div>}
