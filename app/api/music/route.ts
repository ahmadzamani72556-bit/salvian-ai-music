...
async function creditRequest(auth: string, action: "balance" | "consume" | "refund", amount = MUSIC_CREDIT_COST) {
  const response = await fetch(CREDIT_API, { method: "POST", headers: { "Content-Type": "application/json", Authorization: auth }, body: JSON.stringify({ action, amount, description: action === "consume" ? "Pembuatan lagu SALVIAN AI MUSIC" : "Refund pembuatan lagu SALVIAN AI MUSIC" }), cache: "no-store" });
  const data = await response.json().catch(() => ({}));
  return { response, data };
}
async function libraryRequest(auth: string, method: "POST" | "PATCH", body: Record<string, unknown>, query = "") {
  return fetch(`${DATA_API}/salvian_music_projects${query}`, { method, headers: { "Content-Type": "application/json", Authorization: auth, Accept: "application/json", Prefer: "return=representation" }, body: JSON.stringify(body), cache: "no-store" });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === "balance") {
      const credit = await creditRequest(auth, "balance");
      if (!credit.response.ok) return NextResponse.json({ success: false, error: credit.data?.error || "Gagal membaca saldo kredit." }, { status: 401 });
      return NextResponse.json({ success: true, plan: credit.data?.plan || "FREE", credits: Number(credit.data?.credits || 0) });
    }
    ...
    if (action === "status") {
      ...
      try {
        await libraryRequest(auth, "PATCH", { status, audio_url: audio, updated_at: new Date().toISOString(), provider_data: data }, `?task_id=eq.${encodeURIComponent(String(taskId))}`);
    ...
    if (!credit.response.ok || credit.data?.success !== true) return NextResponse.json({ success: false, error: credit.data?.error || "Kredit tidak cukup.", credits: Number(credit.data?.credits || 0) }, { status: credit.response.status === 402 ? 402 : 503 });
    ...
    try {
      ...
      try {
        const libraryResponse = await libraryRequest(auth, "POST", {
          title: String(body?.title || "Salvian AI Song").slice(0, 160),
          lyrics,
          style,
          model: String(body?.model || "auto").slice(0, 80),
          task_id: taskId,
          status,
          audio_url: audio,
          provider_data: data,
      ...
      return NextResponse.json({ success: true, title: String(body?.title || "Salvian AI Song"), taskId, status, audio, url: audio, credits: Number(credit.data?.credits || 0), librarySaved, data });
    } finally {
      if (!providerSucceeded) { try { await creditRequest(auth, "refund"); } catch (refundError) { console.error("MUSIC CREDIT REFUND ERROR", refundError); } }
    }
  } catch (error) {
