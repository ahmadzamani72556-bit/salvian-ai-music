...
// SALVIAN AI MUSIC has its own direct music-generation path.
// SALVIAN AI CREATOR remains a separate application with its own Mureka path.
...
function findAudio(value: unknown, depth = 0): string | null {
  ...
  for (const key of ["audio", "choices", "songs", "outputs", "results", "data"]) {
    if (obj[key]) {
      const found = findAudio(obj[key], depth + 1);
      if (found) return found;
    }
  }
  return null;
...
function findTaskId(value: unknown): string | null {
  ...
  const obj = value as Record<string, unknown>;
  // Mureka's async song generation response uses the top-level `id` as the task ID.
...
async function getCredits(auth: string) {
  return neonRpc(auth, "salvian_get_my_credits");
}

async function consumeCredits(auth: string) {
  return neonRpc(auth, "salvian_consume_credits", {
...
async function refundCredits(auth: string) {
  return neonRpc(auth, "salvian_refund_credits", {
    p_amount: MUSIC_CREDIT_COST,
    p_description: "Refund pembuatan lagu SALVIAN AI MUSIC",
  });
}
...
async function createLibraryProject(auth: string, body: Record<string, unknown>) {
  ...
  if (userId) {
    const insertBody = {
      ...
      status: body.p_status || "preparing",
...
async function patchLibraryProject(auth: string, taskId: string, status: string, audio: string | null, data: unknown) {
  ...
  return fetch(patchUrl, {
    method: "PATCH",
    headers: { "Content-Type": "application/json", Authorization: auth, Accept: "application/json", Prefer: "return=representation" },
    body: JSON.stringify({ status, audio_url: audio, updated_at: new Date().toISOString(), provider_data: data }),
    cache: "no-store",
  });
}

async function callMureka(path: string, init: RequestInit = {}) {
  if (!MUREKA_API_KEY) throw new Error("MUREKA_API_KEY / MUSIC_API_KEY belum tersedia di Vercel SALVIAN AI MUSIC.");
  const response = await fetch(`${MUREKA_BASE_URL}${path}`, {
    ...init,
    headers: {
...
export async function POST(request: NextRequest) {
  ...
  try {
    if (!auth || !/^Bearer\s+/i.test(auth)) {
      return NextResponse.json({ success: false, error: "Silakan login melalui Akun SALVIAN AI terlebih dahulu." }, { status: 401 });
    }
    ...
    if (action === "status") {
      ...
      const { response, data } = await callMureka(`/v1/song/query/${encodeURIComponent(String(taskId))}`, { method: "GET" });
      if (!response.ok) {
        return NextResponse.json({ success: false, taskId: String(taskId), error: data?.error?.message || data?.message || data?.error || `Mureka Query ${response.status}`, data }, { status: response.status });
      }
      ...
      try {
        ...
        console.error("MUSIC LIBRARY STATUS ERROR", libraryError);
      }
      ...
      const result = NextResponse.json({
        ...
        createdAt: Number(data?.created_at || data?.data?.created_at || 0),
        finishedAt: Number(data?.finished_at || data?.data?.finished_at || 0),
        failedReason: data?.failed_reason || data?.data?.failed_reason || null,
        data,
      });
    ...
    if (!credit.response.ok || creditRow?.success !== true) {
      return NextResponse.json({ success: false, error: creditRow?.message || "Kredit tidak cukup.", credits: Number(creditRow?.balance || 0) }, { status: credit.response.status === 402 ? 402 : 503 });
    }
    ...
    try {
      ...
      // Protect the user's provider balance: one Music request creates exactly one song.
      const musicRequest: Record<string, unknown> = {
      ...
      const { response, data } = await callMureka("/v1/song/generate", {
        method: "POST",
      ...
      if (!taskId) {
        ...
        return NextResponse.json({ success: false, error: "Mureka menerima permintaan tetapi tidak mengembalikan Task ID. Kredit akan dikembalikan.", data }, { status: 502 });
      }
      ...
      let libraryError = "";
      try {
        const saved = await createLibraryProject(auth, {
          p_title: String(body?.title || "Salvian AI Song").slice(0, 160),
          p_lyrics: lyrics,
          p_style: style,
          p_model: String(body?.model || "auto").slice(0, 80),
          p_task_id: taskId,
          p_status: status,
          p_audio_url: audio,
      ...
      } catch (libraryErrorCaught) {
        libraryError = libraryErrorCaught instanceof Error ? libraryErrorCaught.message : "Library save gagal";
        console.error("MUSIC LIBRARY SAVE ERROR", libraryErrorCaught);
      }
      ...
      const result = NextResponse.json({
        ...
        status,
        audio,
        credits: Number(creditRow?.balance || 0),
        librarySaved,
        libraryError,
        createdAt: Number(data?.created_at || data?.data?.created_at || 0),
        data,
      ...
      if (!isFinished(status)) result.cookies.set("salvian_generation_task", String(taskId), { path: "/", maxAge: 3600, sameSite: "lax" });
      return result;
    } finally {
      if (!providerSucceeded) {
        try { await refundCredits(auth); }
        catch (refundError) { console.error("MUSIC CREDIT REFUND ERROR", refundError); }
      }
    }
  } catch (error) {
