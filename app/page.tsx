...
export default function HomePage() {
  ...
  const renderProject = () => selectedProject && <section><button onClick={()=>setView("library")} className="mb-5 flex items-center gap-2 text-sm text-zinc-400"><ArrowLeft size={16}/> Kembali ke Library</button><div className="card overflow-hidden"><div className="bg-gradient-to-br from-violet-500/20 via-fuchsia-500/10 to-transparent p-6"><div className="grid h-20 w-20 place-items-center rounded-3xl bg-black/30"><Music2 size={34}/></div><p className="mt-5 text-xs uppercase tracking-[.25em] text-violet-200">SALVIAN AI MUSIC</p><h1 className="mt-2 text-2xl font-extrabold">{selectedProject.title}</h1><p className="mt-2 text-sm text-zinc-400">{selectedProject.status}</p></div><div className="space-y-4 p-5">{selectedProject.audio ? <audio controls src={selectedProject.audio} className="w-full"/> : <button onClick={()=>refreshProject(selectedProject)} disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-violet-400/20 bg-violet-500/10 px-4 py-3 text-sm font-bold text-violet-100"><Play size={17}/> {busy ? "Verifikasi…" : "Verifikasi hasil lagu"}</button>}<div className="rounded-2xl border border-white/8 bg-black/20 p-4"><div className="text-xs text-zinc-500">Gaya &amp; Vokal</div><p className="mt-2 text-sm leading-6 text-zinc-300">{selectedProject.style || "—"}</p></div><div className="rounded-2xl border border-white/8 bg-black/20 p-4"><div className="text-xs text-zinc-500">Lirik</div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-300">{selectedProject.lyrics || "—"}</p></div><button onClick={()=>{removeProject(selectedProject.id);setSelectedProject(null);setView("library")}} className="flex items-center justify-center gap-2 text-xs text-red-300"><Trash2 size={14}/> Hapus project lokal</button></div></div></section>;

  const renderProfile = () => <section><div className="mb-6"><p className="text-xs font-semibold uppercase tracking-[.25em] text-violet-300">Account</p><h1 className="mt-2 text-3xl font-extrabold">Profil</h1></div><div className="card p-6"><div className="flex items-center gap-4"><div className="grid h-16 w-16 place-items-center rounded-full bg-violet-500/15"><UserRound/></div><div><h2 className="font-bold">SALVIAN</h2><p className="text-sm text-zinc-500">{userEmail || "Belum login"}</p></div></div><div className="mt-6 grid grid-cols-2 gap-3"><div className="rounded-2xl border border-white/8 bg-black/20 p-4"><div className="text-xs text-zinc-500">Paket</div><div className="mt-1 font-bold">{plan}</div></div><div className="rounded-2xl border border-white/8 bg-black/20 p-4"><div className="text-xs text-zinc-500">Kredit</div><div className="mt-1 font-bold">{credits === null ? "—" : credits.toLocaleString("id-ID")}</div></div></div>{!token && <button onClick={login} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-extrabold text-black"><LogIn size={17}/> Login melalui Creator</button>}</div></section>;

  const models = ["v5.5 Pro","v5 Pro","v4.5+","v4.5","v4.5-all","Lyrics Model Classic"];

  return <main className="min-h-screen bg-[#07070a] text-zinc-100"><div className="mx-auto min-h-screen max-w-6xl px-4 pb-28 pt-5 sm:px-6"><header className="mb-6 flex items-center justify-between"><div><div className="text-lg font-black tracking-tight">SALVIAN <span className="text-violet-300">AI MUSIC</span></div><div className="text-[11px] text-zinc-500">Create original music with AI</div></div><button onClick={()=>setAdvanced(v=>!v)} className={`rounded-full border px-3 py-2 text-xs font-bold ${advanced ? "border-violet-400/40 bg-violet-500/15 text-violet-200" : "border-white/10 bg-white/5 text-zinc-300"}`}>{advanced ? "Advanced ON" : "Advanced"}</button></header><div className="mb-5 flex items-center justify-between rounded-2xl border border-white/8 bg-white/[.03] px-4 py-3"><span className="text-xs text-zinc-400">{token ? `Akun: ${userEmail || "SALVIAN"}` : "Akun belum terhubung"}</span><span className="text-xs font-bold text-violet-200">{credits === null ? "— kredit" : `${credits.toLocaleString("id-ID")} kredit`} · {plan}</span></div>{view === "create" ? renderCreate() : view === "library" ? renderLibrary() : view === "project" ? renderProject() : renderProfile()}</div>{showModels && <div className="fixed inset-0 z-50 grid place-items-end bg-black/70 p-3 sm:place-items-center"><div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#111116] p-4 shadow-2xl"><div className="mb-3 flex items-center justify-between"><b>Pilih Model</b><button onClick={()=>setShowModels(false)}><X size={18}/></button></div><div className="space-y-2">{models.map(m=><button key={m} onClick={()=>{setModel(m);setShowModels(false)}} className={`w-full rounded-2xl border px-4 py-3 text-left text-sm ${model===m?"border-violet-400/40 bg-violet-500/10":"border-white/8 bg-white/[.03]"}`}>{m}</button>)}</div></div></div>}<nav className="fixed bottom-3 left-1/2 z-40 flex w-[calc(100%-24px)] max-w-lg -translate-x-1/2 items-center justify-around rounded-3xl border border-white/10 bg-[#101015]/95 p-2 shadow-2xl backdrop-blur"><button onClick={()=>setView("create")} className="grid place-items-center gap-1 rounded-2xl px-4 py-2 text-[11px] text-zinc-300"><Home size={18}/>Buat</button><button onClick={()=>setView("library")} className="grid place-items-center gap-1 rounded-2xl px-4 py-2 text-[11px] text-zinc-300"><Library size={18}/>Library</button><button onClick={()=>setView("library")} className="grid place-items-center gap-1 rounded-2xl px-4 py-2 text-[11px] text-zinc-300"><Music2 size={18}/>Projects</button><button onClick={()=>setView("profile")} className="grid place-items-center gap-1 rounded-2xl px-4 py-2 text-[11px] text-zinc-300"><UserRound size={18}/>Profil</button></nav></main>;
}
