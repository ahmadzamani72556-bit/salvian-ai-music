...
export default function HomePage() {
  ...
  const createSong = () => {
    ...
    setTimeout(() => { setBusy(false); setNotice("Project siap. Engine audio akan disambungkan pada tahap generasi berikutnya."); }, 900);
  };
    ...
    <main className="min-h-screen pb-24">
      <header className="glass sticky top-0 z-40">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-white text-black"><Music2 size={21}/></div><div><div className="text-sm font-extrabold tracking-tight">SALVIAN AI</div><div className="text-[11px] text-zinc-400">MUSIC</div></div></div>
          <button onClick={() => setAdvanced(!advanced)} className={`flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold transition ${advanced ? "border-violet-400/50 bg-violet-500/15 text-violet-200" : "border-white/10 bg-white/5 text-zinc-300"}`}><Settings2 size={15}/>{advanced ? "Advanced ON" : "Advanced"}</button>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-5xl px-4 pt-7">
        ...
        <div className="space-y-4">
          <section className="card overflow-hidden">
            <div className="flex items-center justify-between border-b border-white/7 px-4 py-4"><div className="flex items-center gap-2"><Music2 size={18}/><span className="font-bold">Lirik</span></div><button onClick={helpLyrics} disabled={busy} className="flex items-center gap-2 rounded-xl border border-violet-400/20 bg-violet-500/10 px-3 py-2 text-xs font-bold text-violet-200 disabled:opacity-50"><WandSparkles size={14}/> Bantu AI</button></div>
          ...
          <section className="card overflow-hidden">
            ...
            <div className="flex gap-2 overflow-x-auto px-4 pb-4">{examples.map((item,i)=><button key={i} onClick={()=>setStyle(item)} className="shrink-0 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[11px] text-zinc-300">Contoh {i+1}</button>)}</div>
          </section>
          ...
          {advanced && <section className="card p-4"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-bold">Advanced</h2><p className="text-xs text-zinc-500">Kontrol tambahan untuk pengguna berpengalaman.</p></div><Settings2 size={18} className="text-violet-300"/></div><div className="grid gap-3 sm:grid-cols-3">{[["Instrumental","Tanpa vokal"],["Variasi","2 hasil"],["Kreativitas","Balanced"]].map(([a,b])=><div key={a} className="rounded-2xl border border-white/8 bg-black/20 p-4"><div className="text-xs text-zinc-500">{a}</div><div className="mt-1 font-semibold">{b}</div></div>)}</div></section>}
      ...
      {showModels && <div className="fixed inset-0 z-50 grid place-items-end bg-black/70 p-3 sm:place-items-center"><div className="card w-full max-w-md p-4"><div className="mb-4 flex items-center justify-between"><div><h3 className="font-bold">Pilih Model</h3><p className="text-xs text-zinc-500">Model dapat diperluas saat engine tersedia.</p></div><button onClick={()=>setShowModels(false)} className="rounded-full p-2 hover:bg-white/10"><X size={18}/></button></div>{['v5.5 Pro','v5 Pro','v4.5+','v4.5','v4.5-all','Lyrics Model Classic'].map((m,i)=><button key={m} onClick={()=>setShowModels(false)} className={`flex w-full items-center justify-between rounded-xl px-3 py-3 text-left text-sm ${i===0?'bg-violet-500/15 text-violet-200':'hover:bg-white/5'}`}><span>{m}</span>{i===0&&<span className="text-[10px] font-bold">AKTIF</span>}</button>)}</div></div>}

      <nav className="glass fixed bottom-0 left-0 right-0 z-40 border-t border-white/10"><div className="mx-auto grid max-w-5xl grid-cols-4 px-2 py-2">{[[Home,'Buat'],[Library,'Library'],[Music2,'Proyek'],[UserRound,'Profil']].map(([Icon,label],i)=><button key={label as string} className={`flex flex-col items-center gap-1 rounded-xl py-2 text-[10px] ${i===0?'text-white':'text-zinc-500'}`}><Icon size={18}/><span>{label as string}</span></button>)}</div></nav>
    </main>
  );
}
